public class ArrayDequeLegacy<T> {
    /**
    ...
    private void sizeModifier() {
        ...
        // there's no need to enlarge the size!
            ...
            if (rightExtendIndex - 1 < leftExtendIndex + 1) {
                // One side exceeds!
                // WHEN DEFINING VARS, MUST FIGURE OUT ITS MEANING!!!

                if (rightExtendIndex - 1 < (size / 2)) {
                    // Right exceeds!
                    // Copying left ones
                    System.arraycopy(data, leftExtendIndex + 1,
                            tempNew, size / 2 + leftExtendIndex + 1,
                            size / 2 - leftExtendIndex - 1);
                    // Copy right ones
            ...
            } else if (rightExtendIndex - 1 > leftExtendIndex + 1) {
                System.arraycopy(data, leftExtendIndex, tempNew,
    ...
    public T removeFirst() {
        ...
        T result = data[leftExtendIndex + 1];
        data[leftExtendIndex + 1] = null;
        // Must realize that the `leftExtendIndex` is a complete boundary!
        // It can not  be taken to index items!
    ...
    public T removeLast() {
        ...
        T result = data[rightExtendIndex - 1];
    ...
    public T get(int index) {
        if ((leftExtendIndex + index) % size <= rightExtendIndex) {
            return data[(leftExtendIndex + 1 + index) % size];
        } else {
    ...
    public static void main(String[] args){
        ArrayDequeLegacy<Integer> AD = new ArrayDequeLegacy<>();
        // Test add and get
        AD.addFirst(5);
        AD.addLast(6);
        int acquireSum = AD.get(0); // expected : 5
        int sizeTest1 = AD.size(); // expected : 2
        int rmTest1 = AD.removeFirst(); // expected 5
        int rmTest2 = AD.removeLast(); // expected 6
        int sizeTest2 = AD.size(); // expected 0
        boolean emptyTest = AD.isEmpty();
        AD.addLast(0);
        AD.addLast(1);
        AD.addLast(2);
        AD.addLast(3);
        ...
        AD.addLast(8);
        AD.addLast(9);
        int extendTest1 = AD.size;
        int extendTest2 = AD.size();
        int extendRemovalTest1 = AD.removeFirst();
    }
